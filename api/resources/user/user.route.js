...
router.delete('/', auth, async (req, res) => {
  try {
    ...
    if(isEmpty(id)) {
      return res.json(500).json({ success: false, message: 'Invalid request id' })
    }
    ...
    const user = await User.findOneAndUpdate({ _id: id }, { $set: { active: false}}, { new: true});

    return res.status(201).json({ success: true, data: user, message: 'User account deleted successfully' });
